 ...
 *
 * The controller is responsible for receiving input requests from the
 * Application Layer, performing minimal validation, and delegating all business
 * logic to the Service Layer. It must contain NO business logic of its own.
 *
 * Operations supported by this controller: - Quantity comparison - Unit
 * conversion - Addition (with and without target unit) - Subtraction (with and
 * without target unit) - Division
 *
 * Each operation has both a raw "perform" method (returns result directly) and
 * a "demonstrate" method (prints formatted output to the console for demo
 * purposes).
 *
 * @author Developer
 * @version 16.0
...
public class QuantityMeasurementController {
	...
	/**
	 * Logger for recording controller-level events. Uses SLF4J-compatible
	 * java.util.logging as configured by Logback in pom.xml.
	 */
	...
	/**
	 * Delegates an addition request to the service layer. Result is returned in the
	 * same unit as the first operand.
	 *
	 * @param thisQuantityDTO first quantity
	 * @param thatQuantityDTO second quantity
	...
	/**
	 * Delegates a subtraction request to the service layer. Result is returned in
	 * the same unit as the first operand.
	 *
	 * @param thisQuantityDTO first quantity
	 * @param thatQuantityDTO second quantity

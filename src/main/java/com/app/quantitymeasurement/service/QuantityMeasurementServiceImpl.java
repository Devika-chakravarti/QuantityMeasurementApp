 ...
 *
 * Service layer implementation responsible for executing quantity measurement
 * business operations. Implements IQuantityMeasurementService and contains the
 * core logic for: - Quantity comparison - Unit conversion - Addition -
 * Subtraction - Division
 *
 * The service layer acts as the business logic layer in the N-Tier architecture
 * and interacts with the repository layer (injected via constructor) to store
 * operation results. Since the repository is injected through the interface,
 * the service can work seamlessly with both QuantityMeasurementCacheRepository
 * and QuantityMeasurementDatabaseRepository — no changes to service logic are
 * needed.
 *
 * Internally the service converts QuantityDTO objects into QuantityModel
 * objects to perform calculations using measurable unit implementations.
 *
 * @author Developer
 * @version 16.0
 * @since 1.0
...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
	...
	/**
	 * Repository used for persisting measurement operation results. Injected at
	 * construction time — supports both cache and database repositories.
	 */
	 ...
	 *
	 * Dependency Injection pattern: the caller supplies the repository
	 * implementation, so the service never creates its own persistence layer.
	 *
	...
	public QuantityMeasurementServiceImpl(IQuantityMeasurementRepository repository) {
		...
		logger.info(
				"QuantityMeasurementServiceImpl initialized with repository: " + repository.getClass().getSimpleName());
	}
	 ...
	 * @throws IllegalArgumentException if the quantities belong to different
	 *                                  categories
	 */
	@Override
	public boolean compare(QuantityDTO thisQuantityDTO, QuantityDTO thatQuantityDTO) {
		...
		if (!q1.getUnit().getMeasurementType().equals(q2.getUnit().getMeasurementType())) {
			throw new IllegalArgumentException("Cannot compare different measurement categories");
		}
		 ...
		 */
		repository
				.save(new QuantityMeasurementEntity(q1, q2, Operation.COMPARE.name(), result ? "Equal" : "Not Equal"));
	...
	public QuantityDTO convert(QuantityDTO thisQuantityDTO, QuantityDTO targetUnitDTO) {
		...
		repository.save(new QuantityMeasurementEntity(source, target, Operation.CONVERT.name(), resultModel));
	 ...
	 */
	private <U extends IMeasurable> double convertTemperatureUnit(QuantityModel<U> source, U targetUnit) {
	 ...
	 * @throws IllegalArgumentException      if the operands are from different
	 *                                       categories
	 * @throws UnsupportedOperationException if the measurement type does not
	 *                                       support arithmetic
	 */
	@Override
	public QuantityDTO add(QuantityDTO thisQuantityDTO, QuantityDTO thatQuantityDTO, QuantityDTO targetUnitDTO) {
		...
		repository.save(new QuantityMeasurementEntity(q1, q2, Operation.ADD.name(), resultModel));
	...
	/**
	 * Subtracts the second quantity from the first and converts the result to the
	 * target unit.
	 *
	 * @param thisQuantityDTO first quantity
	 * @param thatQuantityDTO second quantity
	...
	public QuantityDTO subtract(QuantityDTO thisQuantityDTO, QuantityDTO thatQuantityDTO, QuantityDTO targetUnitDTO) {
		...
		repository.save(new QuantityMeasurementEntity(q1, q2, Operation.SUBTRACT.name(), resultModel));
	...
	public double divide(QuantityDTO thisQuantityDTO, QuantityDTO thatQuantityDTO) {
		...
		repository.save(new QuantityMeasurementEntity(q1, q2, Operation.DIVIDE.name(), String.valueOf(result)));
	...
	/**
	 * Converts a QuantityDTO into an internal QuantityModel with the appropriate
	 * unit.
	 *
	 * @param quantity incoming DTO
	 * @return QuantityModel populated with value and unit
	 * @throws IllegalArgumentException if quantity is null or unit type is
	 *                                  unsupported
	 */
	...
	private QuantityDTO getQuantityDTO(QuantityModel<IMeasurable> quantity) {
		...
		IMeasurableUnit dtoUnit = getDTOUnit(quantity.getUnit().getMeasurementType(), quantity.getUnit().getUnitName());
		return new QuantityDTO(quantity.getValue(), dtoUnit);
	}

	/**
	 * Resolves the IMeasurable unit instance from measurement type and unit name
	 * strings.
	 *
	...
	/**
	 * Resolves the QuantityDTO.IMeasurableUnit enum from measurement type and unit
	 * name.
	 *
	 * @param measurementType category (e.g., "LengthUnit")
	 * @param unit            unit name (e.g., "FEET")
	 ...
	 *
	 * Checks: 1. Neither operand is null. 2. Both operands belong to the same
	 * measurement category. 3. The measurement category supports arithmetic (not
	 * temperature). 4. If a target unit is required, it is not null.
	 *
	...
	private <U extends IMeasurable> void validateArithmeticOperands(QuantityModel<U> q1, QuantityModel<U> q2,
		...
		/*
		 * Temperature arithmetic is not physically meaningful (you cannot "add"
		 * temperatures), so it is explicitly blocked here.
		 */
		if (type1.equals("TemperatureUnit")) {
			throw new UnsupportedOperationException("Arithmetic operations are not supported for temperature units");
		}
	 ...
	 *
	 * Both operands are first converted to their base units before the operation is
	 * applied. This ensures accuracy regardless of the input units.
	 *
	 * @param q1        first operand
	 * @param q2        second operand
	 ...
	 */
	private <U extends IMeasurable> double performArithmetic(QuantityModel<U> q1, QuantityModel<U> q2,

 ...
 * QuantityModel
 *
 * Generic POJO model class for representing a quantity with its associated
 * measurable unit.
 *
 * This model is primarily used in the service layer for performing quantity
 * operations such as: - Unit conversion - Quantity comparison - Arithmetic
 * operations
 *
 * The class stores a numeric value and its corresponding measurement unit that
 * implements IMeasurable. Using generics ensures type safety so that only valid
...
public class QuantityModel<U extends IMeasurable> {
	...
	public String toString() {
		return String.format("%s %s", Double.toString(value).replaceAll("\\.0+$", ""), unit.getUnitName());
	}
}
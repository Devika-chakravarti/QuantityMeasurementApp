 ...
 *
 * This entity is used to persist and retrieve measurement operation history
 * from the database. It stores operands (thisQuantity and thatQuantity), the
 * operation type, and the result (as a model, string, or error message).
 *
 * Stores: - First and second operand values, units, and measurement types -
 * Operation type (COMPARE, CONVERT, ADD, SUBTRACT, DIVIDE) - Result value,
 * unit, and measurement type (for arithmetic operations) - Result string (for
 * comparison/conversion results like "Equal") - Error information (for failed
 * operations)
 *
...
public class QuantityMeasurementEntity implements java.io.Serializable {
	...
	/**
	 * String result used for operations like COMPARE ("Equal" / "Not Equal") or
	 * DIVIDE (numeric ratio as string).
	 */
	...
	/**
	 * Constructor for comparison or conversion operations where the result is a
	 * string.
	 *
	 ...
	 *
	 * Initializes operand information and operation type. Validates that neither
	 * operand is null.
	 *
	 * @param thisQuantity first operand
	 * @param thatQuantity second operand
	...
	public boolean equals(Object obj) {
		...
		return Math.abs(this.thisValue - other.thisValue) < 1e-6 && Math.abs(this.thatValue - other.thatValue) < 1e-6
				&& this.thisUnit.equals(other.thisUnit) && this.thatUnit.equals(other.thatUnit)
	 ...
	 *
	 * Includes operation, operand details, and result or error information. Used
	 * for logging, debugging, and displaying stored measurements.
	 *
	...
	public String toString() {
		...
		sb.append(isError ? "[ERROR] " : "[SUCCESS] ").append("operation=").append(operation);

		sb.append(", operand1=").append(thisValue).append(" ").append(thisUnit).append(" ").append(thisMeasurementType);

		sb.append(", operand2=").append(thatValue).append(" ").append(thatUnit).append(" ").append(thatMeasurementType);
		...
		if (isError) {
			...
			sb.append(", result=").append(resultValue).append(" ").append(resultUnit).append(" ")

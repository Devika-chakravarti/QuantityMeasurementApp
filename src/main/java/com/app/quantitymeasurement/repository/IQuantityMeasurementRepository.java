 ...
 * quantity measurement entities. It abstracts the persistence mechanism so that
 * the service layer does not depend on implementation details of storage.
 *
 * Implementations of this interface may store data using: - In-memory cache
 * (QuantityMeasurementCacheRepository) - Database storage via JDBC
 * (QuantityMeasurementDatabaseRepository)
 *
 * UC16 enhancements over UC15: - Added getMeasurementsByOperation() for
 * filtering by operation type - Added getMeasurementsByType() for filtering by
 * measurement category - Added getTotalCount() for reporting and monitoring -
 * Added deleteAll() for test isolation and state reset - Added default methods
 * for pool statistics and resource cleanup
 *
...
public interface IQuantityMeasurementRepository {
	 ...
	 *
	 * Useful for filtering the history by category (e.g., "LengthUnit",
	 * "WeightUnit").
	 *
	 ...
	 *
	 * Default implementation returns a message indicating pool statistics are not
	 * available for this repository type. Database-backed implementations should
	 * override this to return meaningful pool metrics.
	 *
	...
	default void releaseResources() {
		/*
		 * Default implementation does nothing — override in resource-holding
		 * implementations
		 */
	}
}
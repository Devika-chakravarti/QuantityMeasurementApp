 ...
 *
 * This class implements the IQuantityMeasurementRepository interface and
 * provides methods to interact with a relational database for storing and
 * retrieving quantity measurement data. It uses JDBC for database operations
 * and a connection pool for efficient resource management. The repository
 * handles CRUD operations for QuantityMeasurementEntity objects and includes
 * methods to query measurements by operation type and measurement type. It also
 * includes error handling using a custom DatabaseException to encapsulate any
 * database-related issues. The repository is designed to be used by the service
 * layer of the application to persist the results of quantity measurement
 * operations and to retrieve historical data for analysis and reporting. It
 * ensures that database connections are properly managed and that resources are
 * released after use to prevent leaks and ensure optimal performance.
 *
 * @author Developer
 * @version 16.0
 * @since 16.0
...
public class QuantityMeasurementDatabaseRepository implements IQuantityMeasurementRepository {
	...
	/*
	 * Pre-compiled SQL query constants. Parameterized queries (?) prevent SQL
	 * injection by treating user-supplied data as literal values, never as
	 * executable SQL.
	 */

	/**
	 * SQL INSERT statement for persisting a new measurement entity. All fields
	 * except id (AUTO_INCREMENT) and timestamps (DEFAULT) are explicitly set.
	 */
	...
	/**
	 * SQL CREATE TABLE for the main measurement entity table. Uses IF NOT EXISTS so
	 * it is safe to call on every startup without errors.
	 */
	private static final String CREATE_TABLE_SQL = "CREATE TABLE IF NOT EXISTS quantity_measurement_entity ("
			+ "  id BIGINT AUTO_INCREMENT PRIMARY KEY," + "  this_value DOUBLE NOT NULL,"
			+ "  this_unit VARCHAR(50) NOT NULL," + "  this_measurement_type VARCHAR(50) NOT NULL,"
			...
			+ "  result_measurement_type VARCHAR(50)," + "  result_string VARCHAR(255),"
			+ "  is_error BOOLEAN DEFAULT FALSE," + "  error_message VARCHAR(500),"
	...
	/**
	 * Private constructor — initializes the connection pool and sets up the
	 * database schema.
	 *
	 * Called only once via getInstance(). Any SQLException during pool
	 * initialization is wrapped in a DatabaseException so it propagates as an
	 * unchecked exception.
	 */
	private QuantityMeasurementDatabaseRepository() {
		try {
			...
			/*
			 * Ensure the schema exists before any operations are attempted. This is safe to
			 * call on every startup — the CREATE TABLE uses IF NOT EXISTS.
			 */
			initializeDatabase();
	 ...
	 * quantity_measurement_entity table if it does not already exist. It is called
	 * during the construction of the repository to ensure that the database is
	 * ready for use. The method uses a connection from the connection pool to
	 * execute the SQL statement for creating the table. Error handling is included
	 * to catch any SQL exceptions that may occur during the execution of the schema
	 * initialization and logs any errors encountered.
	 *
	 * Note: In a production environment, database schema management is typically
	 * handled separately using migration tools like Flyway or Liquibase. This
	 * method is included here for simplicity and testing purposes to ensure that
	 * the necessary table exists before running any database operations.
	 */
	 ...
	 *
	 * Thread-safe using synchronized to prevent double-initialization under
	 * concurrency.
	 *
	...
	/**
	 * Saves a QuantityMeasurementEntity to the database. This method uses a
	 * prepared statement to insert the entity's data into the
	 * quantity_measurement_entity table. It handles SQL exceptions by catching them
	 * and throwing a custom DatabaseException with a meaningful message. The method
	 * also ensures that all database resources are properly closed after use to
	 * prevent leaks. Logging is included to track successful saves and any errors
	...
	public void save(QuantityMeasurementEntity entity) {
		...
		try {
			...
			/*
			 * Bind all entity fields to the parameterized query. Parameterized binding
			 * prevents SQL injection — data is never concatenated into SQL.
			 */
	...
	/**
	 * Retrieves all QuantityMeasurementEntity instances from the database. This
	 * method executes a SQL query to select all records from the
	 * quantity_measurement_entity table, ordered by creation date in descending
	 * order. It uses a statement to execute the query and maps the result set to a
	 * list of QuantityMeasurementEntity objects. The method includes error handling
	 * to catch any SQL exceptions that may occur during the database interaction
	 * and rethrows them as DatabaseException with a meaningful message. Finally, it
	 * ensures that all database resources are properly closed to prevent leaks.
	 * Logging is included to track the number of measurements retrieved and any
	 * errors that occur during the operation.
	 *
	 * @return a list of all QuantityMeasurementEntity instances retrieved from the
	 *         database
	...
	/**
	 * Get measurements by operation type. This method retrieves all quantity
	 * measurement entities from the database that match the specified operation
	 * type (e.g., "Addition", "Subtraction", "Multiplication", "Division"). It uses
	 * a prepared statement to execute the query and maps the result set to a list
	 * of QuantityMeasurementEntity objects. The method also includes error handling
	 * to catch any SQL exceptions that may occur during the database interaction
	 * and rethrows them as DatabaseException with a meaningful message. Finally, it
	 * ensures that all database resources are properly closed to prevent leaks.
	 *
	 ...
	 * This method retrieves all quantity measurement entities from the database
	 * that match the specified measurement type. It uses a prepared statement to
	 * execute the query and maps the result set to a list of
	 ...
	 * total number of quantity measurement entities in the database.
	 *
	...
	/**
	 * Delete all measurements (useful for testing). This method executes a SQL
	 * query to delete all quantity measurement entities from the database.
	 *
	...
	public String getPoolStatistics() {
		return "ConnectionPool{" + "available=" + connectionPool.getAvailableConnectionCount() + ", used="
				+ connectionPool.getUsedConnectionCount() + ", total=" + connectionPool.getTotalConnectionCount() + "}";
	}

	/**
	 * Release resources held by the repository, such as closing database
	 * connections.
	 */
	@Override
	...
	private QuantityMeasurementEntity mapResultSetToEntity(ResultSet rs) throws SQLException {
		/*
		 * Build operand QuantityModel-like data from columns — we reconstruct the
		 * entity fields directly since we store primitive columns, not serialized
		 * objects.
		 */
		...
		Double thatValue = rs.getObject("that_value") != null ? rs.getDouble("that_value") : null;
		 ...
		 * stores flat columns rather than nested QuantityModel objects. The no-arg
		 * constructor is used here; all fields are set below.
		 */
		...
		entity.resultValue = rs.getObject("result_value") != null ? rs.getDouble("result_value") : null;
	 ...
	 *
	 * Overloaded helper for operations that do not produce a ResultSet (INSERT,
	 * DELETE).
	 *
	 * @param stmt Statement to close (may be null)
	 * @param conn Connection to release back to the pool (may be null)
	...
	public static void main(String[] args) {
		try {
			...
			logger.info("Total records: " + repo.getTotalCount());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

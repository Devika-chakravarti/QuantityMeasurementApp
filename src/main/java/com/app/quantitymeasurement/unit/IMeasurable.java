 ...
 *
 * This interface defines the core contract that all measurement unit types must
 * implement, such as LengthUnit, VolumeUnit, WeightUnit, and TemperatureUnit.
 *
 * Implementations of this interface provide the ability to: - Retrieve the unit
 * name - Convert values to the base unit of a measurement type - Convert values
 * from the base unit to a specific unit - Identify the measurement category -
 ...
 * across different measurement categories without depending on specific unit
 * enums.
 *
 * @author Developer
 * @version 16.0
...
public interface IMeasurable {
	...
	/**
	 * Returns whether this unit supports arithmetic operations. Default: true for
	 * all units that implement SupportsArithmetic.
	 */
	...
	/**
	 * Validates that this unit supports the given operation. Throws
	 * UnsupportedOperationException if arithmetic is not supported.
	 *
	 * @param operationName name of the operation being attempted
	 */
	default void validateOperationSupport(String operationName) {
		if (!supportsArithmetic()) {
			throw new UnsupportedOperationException(getMeasurementType() + " does not support " + operationName);
		}
	}

 ...
 * ConnectionPool
 *
 * ConnectionPool class manages a pool of database connections for efficient
 * reuse. It initializes a specified number of connections based on the
 * configuration and provides methods to acquire and release connections. The
 * pool ensures that the number of active connections does not exceed the
 * configured pool size and handles connection creation and closure gracefully.
 * It also provides methods to retrieve pool statistics for monitoring purposes.
 *
 * The ConnectionPool class is designed to be a singleton, ensuring that only
 * one instance of the connection pool exists throughout the application. This
 * design allows for centralized management of database connections. The class
 * uses JDBC for connection management and includes error handling to manage
 * exceptions that may occur during connection creation and closure.
 *
 * @author Developer
 * @version 16.0
 * @since 16.0
...
public class ConnectionPool {
	...
	/**
	 * Private constructor to initialize the connection pool based on the
	 * configuration.
	 *
	 * Reads all database settings from ApplicationConfig and pre-creates the
	...
	private ConnectionPool() throws SQLException {
		...
		/*
		 * Load all database configuration values from application.properties. Default
		 * values are provided for each setting in case they are absent.
		 */
	...
	/**
	 * Initializes the connection pool by creating the specified number of
	 * connections.
	 *
	 ...
	 *
	 * Uses DriverManager to open a fresh JDBC connection with the configured URL,
	...
	public synchronized Connection getConnection() throws SQLException {
		/*
		 * If there are available connections, take the first one from the list and move
		 * it to the used connections list.
		 */
		if (!availableConnections.isEmpty()) {
			Connection conn = availableConnections.remove(0);

			/*
			 * Validate the connection is still alive before handing it out. If it is stale,
			...
			logger.fine("Connection acquired. Available: " + availableConnections.size() + ", Used: "
					+ usedConnections.size());
			return conn;
		}

		/*
		 * No available connections - if we have not yet reached the pool limit, create
		 ...
		 */
		throw new SQLException("Connection pool exhausted. All " + poolSize + " connections are in use.");
	}

	/**
	 * Releases a connection back to the pool. The connection is moved from the used
	 * connections list to the available connections list. If the connection is
	...
	public synchronized void releaseConnection(Connection connection) {
		...
		/*
		 * Move the connection from used back to available so other threads can acquire
		 * it.
		 */
		usedConnections.remove(connection);
		availableConnections.add(connection);
		logger.fine(
				"Connection released. Available: " + availableConnections.size() + ", Used: " + usedConnections.size());
	}
	 ...
	 * It executes a simple query and returns true if the query executes
	 * successfully.
	 *
	...
	public boolean validateConnection(Connection connection) {
		try (var statement = connection.createStatement()) {
			statement.execute(this.testQuery);
			return true;
	 ...
	 * application is shutting down to ensure that all database connections are
	 * properly closed. It iterates through both the available and used connections
	 * and attempts to close each one, logging any exceptions that occur during the
	 * closure process.
	 */
	public synchronized void closeAll() {
		...
		/*
		 * Then close any connections still in use.
		 */
		...
		/*
		 * Reset the singleton so a fresh pool can be created if needed.
		 */
		instance = null;
	...
	public String toString() {
		return "ConnectionPool{" + "poolSize=" + poolSize + ", available=" + availableConnections.size() + ", used="
				+ usedConnections.size() + "}";
	}
